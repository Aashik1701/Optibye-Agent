...
class EMSQueryEngine:
    ...
    def get_latest_readings(self, query):
        ...
        try:
            ...
            response += f"⚡ Voltage: {latest_doc.get('voltage_(v)', latest_doc.get('voltage', 'N/A'))} V\n"
            response += f"🔌 Current: {latest_doc.get('current_(a)', latest_doc.get('current', 'N/A'))} A\n"
            response += f"📈 Power Factor: {latest_doc.get('power_factor', 'N/A')}\n"
            response += f"💡 Active Power: {latest_doc.get('active_power_(kw)', latest_doc.get('active_power', 'N/A'))} kW\n"
            response += f"🔋 Energy Consumed: {latest_doc.get('energy_consumed', 'N/A')} kWh\n"
    ...
    def get_averages(self, query):
        ...
        try:
            ...
            response += f"💡 Average Active Power: {avg_data.get('avg_active_power', 0):.2f} kW\n"
            ...
            if is_energy_report:
                ...
                try:
                    ...
                    if energy_result:
                        ...
                        response += f"⚡ Total Energy Consumed: {data.get('total_energy', 0):.2f} kWh\n"
                        response += f"💡 Average Power: {data.get('avg_power', 0):.2f} kW\n"
                        response += f"📈 Peak Power: {data.get('max_power', 0):.2f} kW\n"
                        response += f"🔌 Average Voltage: {data.get('avg_voltage', 0):.1f} V\n"
                        response += f"🔋 Average Current: {data.get('avg_current', 0):.1f} A\n"
                        response += f"📊 Power Factor: {data.get('avg_power_factor', 0):.3f}\n"
                        response += f"📋 Total Readings: {data.get('count', 0):,}\n"
                
                except Exception as e:
                    response += f"⚠️ Error calculating energy metrics: {str(e)}\n"
                
                # Add anomaly information
                response += f"\n⚠️ Anomalies Detected: {summary.get('anomaly_count', 0)}\n"
    ...
    def get_energy_info(self, query):
        ...
        try:
            ...
            response += f"💡 Average Power: {avg_power:.2f} kW\n"
            response += f"📈 Peak Power: {max_power:.2f} kW\n"
            response += f"🔋 Total Energy Consumed: {total_energy:.2f} kWh\n"
            response += f"📊 Average Energy per Reading: {avg_energy:.2f} kWh\n"
